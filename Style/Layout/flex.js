 ...
 * 弹性盒子布局模型
 *
 * 除传统float和inline-block以外的又一布局利器，简单易用。
 * 优点是简单直观，可以实现各种布局难题，如水平居中，垂直居
 * 中等；缺点是兼容性问题，需要支持IE10+。所以推荐在兼容性
 * 要求不高的地方使用，入后台管理系统，mobile端。
 *
 * 简单使用，只需要在给容器定义`display: flex`就可以了。
 * 一般来说下面3个属性比较常用：
 *
 * 1. justify-content 主轴位置的排列方式
 * 2. align-items 容器内子元素的排列方式
 * 3. flex-flow 这是个组合属性，组合了`flex-direction`
 *    和`flex-wrap`，前者设定主轴是x还是y，后者设定包裹方
 *    式
 *
 * 如垂直居中:
 * ```css
 ...
 * }
 * 
 * 当然，我们还可以对每个子元素设定自己的行为。这里常用的就
 * 是`flex`这个属性。`flex`也是个组合属性，由`flex-grow`，
 * `flex-shrink`，`flex-basic`组成，分别代表了生长，收缩
 * 与基本尺寸。常用方式如下
 *
 * 每个子元素尺寸平均:
 * ```css
